...
//#define N_MAX_loc  (1 << 18)		// maximum 256KB per node !!!
#define N_MAX_loc  (1 << 20)		// maximum 1MB per node !!!
//#define N_MAX_loc  (1 << 22)		// maximum 4MB per node !!!
...
int main(int argc, char *argv[]){
	...
	MPI_Comm_size(MPI_COMM_WORLD, &n_proc);	
        ...
        printf("Rank of the processor %04d on %s \n", myRank, processor_name);
...
#else
#ifdef FOURTH
		std::ifstream ifs("phi-CPU4.cfg");
#endif
#ifdef SIXTH
		std::ifstream ifs("phi-CPU6.cfg");
#endif
#ifdef EIGHTH
		std::ifstream ifs("phi-CPU8.cfg");
#endif
#endif
...
#else
#ifdef FOURTH
    printf("Begin the calculation of phi-CPU4 program on %03d processors\n", n_proc); 
#endif
#ifdef SIXTH
    printf("Begin the calculation of phi-CPU6 program on %03d processors\n", n_proc); 
#endif
#ifdef EIGHTH
    printf("Begin the calculation of phi-CPU8 program on %03d processors\n", n_proc); 
#endif
#endif
...
*/
		...
		double t_tot = t_scan + t_pred + t_preda + t_jsend + t_force + t_comm + t_corr;
		...
		printf("preda:%12.4E%12.4E%12.4E\n", t_preda, t_preda/Timesteps*1.e6, t_preda/t_tot);
